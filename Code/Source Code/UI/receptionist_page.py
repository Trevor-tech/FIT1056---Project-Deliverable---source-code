...
class ReceptionistPage(tk.Tk):
    ...
    def widgets(self):
        # A welcome label
        welcome_label = tk.Label(self, text=f"Welcome, {self.receptionist.username}!", font=("Arial", 24, "bold"))
        welcome_label.pack( pady=10)

        # Add buttons for receptionist actions
        tk.Button(self, text="Manage Enrolments", command = self.show_manage_enrolments, font=("Arial", 18, "bold")).pack(pady=5)
...
class ManageEnrolments(tk.Frame):
    def __init__(self, parent):
        ...
        self.unenroll_button = tk.Button(self, text="Unenroll Student", command=self.unenroll_selected_student)
    ...
    def load_enrolments(self):
        self.enrolment_listbox.delete(0, tk.END)  # Clear existing entries
        self.enrollments = []  # Store enrollment data for easy access
        try:
            ...
            with open(file_path, "r") as f:
                ...
                for line in lines:
                    ...
                    if len(parts) == 3:  # Old format without credit
                        ...
                        continue
                    self.enrollments.append((course_id, name, date, credit))
    ...
    def unenroll_selected_student(self):
        selected_indices = self.enrolment_listbox.curselection()
        if not selected_indices:
            messagebox.showwarning("No Selection", "Please select an enrollment to unenroll.")
            return

        selected_index = selected_indices[0]
        enrollment = self.enrollments[selected_index]
        course_id, name, date, credit = enrollment

        confirm = messagebox.askyesno("Confirm Unenrollment", 
                                      f"Are you sure you want to unenroll:\n\nStudent: {name}\nCourse ID: {course_id}\nEnrollment Date: {date}")
        if confirm:
            self.enrollments.pop(selected_index)
            self.save_enrollments()
            self.load_enrolments()  # Reload the list
            messagebox.showinfo("Success", f"Student {name} has been unenrolled from course {course_id}.")
        ...
        try:
            with open(file_path, "w") as f:
                for enrollment in self.enrollments:
                    f.write(f"{','.join(enrollment)}\n")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save enrollments: {str(e)}")